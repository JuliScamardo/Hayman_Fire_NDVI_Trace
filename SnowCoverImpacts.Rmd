...
ndsi <-read_csv('data/hayman_ndsi.csv') %>%
  ...
  filter(!is.na(burned),
         !is.na(unburned)) %>%
  gather(.,key='site',value='ndsi',-DateTime) # For this analysis we want the data in long format
...
q_means <- q_water_year %>%
  ...
  mutate(count = n()) %>%
  ungroup() %>%
  filter(count > max(count)*0.8) %>% #want to only include years with at least 80% coverage
  group_by(wtr_yr) %>%
...
```{r, echo = FALSE}
q_snow <- inner_join(snow_means, q_means, by = 'wtr_yr')

ggplot(q_snow, aes(x = mean_ndsi, y = median_q, color = wtr_yr)) + 
  geom_point()+
...
```{r, echo = FALSE}
ggplot(q_snow, aes(x = max_ndsi, y = mean_q, color = wtr_yr)) + 
  geom_point() + 
  xlab('Max Snow Cover')+
